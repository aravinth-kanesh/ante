...
def auth_cookies(client, email="tts@example.com"):
    res = client.post(
...
def test_voices_reports_unavailable_without_the_model(client, monkeypatch):
    ...
    body = client.get("/api/speech/voices", cookies=auth_cookies(client)).json()
...
def test_say_returns_wav_audio(client, monkeypatch):
    ...
    res = client.post(
        "/api/speech/say", cookies=auth_cookies(client), json={"text": "Hello", "voice": "bf_emma"}
    )
...
def test_say_503_without_the_model(client, monkeypatch):
    monkeypatch.setattr(tts, "available", lambda: False)
    res = client.post("/api/speech/say", cookies=auth_cookies(client), json={"text": "Hello"})
    assert res.status_code == 503


def test_say_rejects_empty_text(client, monkeypatch):
    monkeypatch.setattr(tts, "available", lambda: True)
    res = client.post("/api/speech/say", cookies=auth_cookies(client), json={"text": "  "})
